...
function renderCalender() {
  ...
  for (let i = 0; i < $allDates.length; i++) {
    // 조건추가 : 2022년 8월이고,
    if (month === new Date().getMonth() && $allDates[i].innerText == today) {
      $allDates[i].classList.add("its-today");
    }
  }

  // todo
  const dateBtns = $dates.querySelectorAll("li a");
  for (const dateBtn of dateBtns) {
    dateBtn.addEventListener("click", (e) => {
      e.preventDefault();
      const date = e.target.textContent;
      $todoDate.textContent = `Day ${date}`;
    });
  }
}
